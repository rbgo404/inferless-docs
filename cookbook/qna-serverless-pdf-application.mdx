...
inferless init
```
2. __Upload Custom Runtime:__ Use the following command to upload your [custom runtime](https://github.com/inferless/Document-RAG-Upload/blob/main/inferless-runtime-config.yaml).
```bash
...
build:
  ...
  python_packages:
    ...
    - "accelerate==0.27.2"
```
3. __Deploy Model:__ Execute the following command to deploy and monitor the build logs on Inferless.
```bash
inferless deploy
```
#### PDF Q&A Deployment:
...
inferless init
```
2. __Upload Custom Runtime:__ Use the following command to upload your [custom runtime](https://github.com/inferless/Document-RAG-QnA/blob/main/inferless-runtime-config.yaml).
```bash
inferless runtime upload
```
Here's the custom runtime for the application:
