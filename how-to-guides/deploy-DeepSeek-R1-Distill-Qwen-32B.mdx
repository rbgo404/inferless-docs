...
### Input Schema
Using Pydantic with the inferless Python client allows you to define structured input schemas directly in your code. 
To do so, use the `@inferless.request` decorator to specify input parameters.
...
The input parameter of the `infer` function is now typed as the Pydantic `RequestObjects` class. 
This ensures that input values are validated and accessible directly as attributes.

Then use the `ResponseObjects` class in the infer function to structure the output of your model.
