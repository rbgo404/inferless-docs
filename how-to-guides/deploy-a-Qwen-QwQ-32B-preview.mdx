...
```bash
inferless remote-run app.py -c inferless-runtime-config.yaml --prompt "What is Deep Learning?" --max_tokens "512"
```

You can pass the other input parameters in the same way (e.g., `--repetition_penalty`, `--temperature`, etc.) as long as your code expects them in the `inputs` dictionary.
