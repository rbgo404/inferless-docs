...
class InferlessPythonModel:
    ...
    def infer(self, request: RequestObjects) -> ResponseObjects:
        ...
        input_text = self.pipe.tokenizer.apply_chat_template(messages, tokenize=False, add_generation_prompt=True)
        out = self.pipe(input_text, max_new_tokens=request.max_new_tokens, do_sample=request.do_sample, top_p=request.top_p,temperature=request.temperature)
...
You can pass the other input parameters in the same way (e.g., `--temperature`, `--top_p`, etc.) as long as your code expects them in the `inputs` dictionary.
