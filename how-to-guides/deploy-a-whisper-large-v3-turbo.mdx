...
You can pass the input parameters in the same way (e.g., `--audio_url`, `--language`, etc.) as long as your code expects them in the `inputs` dictionary.

```bash
inferless remote-run app.py -c inferless-runtime-config.yaml --audio_url "https://raw.githubusercontent.com/rbgo404/Files/refs/heads/main/jeanNL.mp3" --language "english"
```
