...
### Steps to import model file from Gitlab
...
<Frame caption = "Click on Add Model">![](/images/gitlab1.png)</Frame>
...
* Since we are using a model from Git, select `Repo(Custom Code)`as the method of upload.

* To proceed with the upload, you will need to connect your `Gitlab account`. This is a mandatory step as this helps us in downloading the file from your Gitlab if the repo you are pulling is from an open account.

![](/images/gitlab3.png)

* If you are using Inferless for the first time and have not added any providers before, click on the `"Link Account" `button (+).
...
* Once you click submit, the model import process will start.  
   ...
   * You can then select the model and go to `-> API -> Inference Endpoint details. `Here you will find the API endpoints that can be called. You can click on the copy button on the right and call your model after modifying the input details.  
